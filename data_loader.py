...
def prepare_training_data(version=1, data_dir='Data', options={}):
    if version == 2:
        ...
        t_q_json_file = join(data_dir, 'training_questions.json')
        t_a_json_file = join(data_dir, 'training_annotations.json')

        v_q_json_file = join(data_dir, 'validation_questions.json')
        v_a_json_file = join(data_dir, 'validation_annotations.json')
...
def main():
    ...
    parser.add_argument('--max_length', type=int, default=15,
                        help='VQA data version')
    parser.add_argument('--train_on_val', type=bool, default=True,
                        help='VQA data version')
